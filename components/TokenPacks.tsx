    ...
    >
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        {packs.map((p, idx) => (
          <div
            key={idx}
            className="rounded-2xl border border-[rgba(200,164,93,0.22)] bg-black/5 p-6 flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between">
                <h3 className="font-serif text-xl">{p.name}</h3>
                {p.badge && (
                  <span className="text-xs px-2 py-1 rounded-full bg-gold-500 text-ink">
                    {p.badge}
                  </span>
                )}
              </div>
              <p className="mt-2 text-5xl font-serif text-gold-500">
                {p.tokens}
            ...
            <button
              className="mt-6 inline-flex items-center justify-center rounded-full border border-[rgba(200,164,93,0.4)] hover:border-[rgba(200,164,93,0.6)] bg-black/30 hover:bg-black/40 px-5 py-3 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-500"
              aria-disabled="true"
